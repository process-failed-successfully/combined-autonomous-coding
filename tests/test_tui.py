...
class TestTUI(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        # Mock KnowledgeManager in tui (if used) and tui_knowledge_graph
        ...
        # Also patch run_ask_logic in tui_knowledge_graph
        self.patcher_ask_kg = patch("shared.tui_knowledge_graph.run_ask_logic", new_callable=AsyncMock)
        self.mock_ask_kg = self.patcher_ask_kg.start()

    def tearDown(self):
        self.patcher_db.stop()
        self.patcher_km.stop()
        self.patcher_ask.stop()
        self.patcher_ask_kg.stop()
    ...
    async def test_knowledge_tab(self, mock_services_tab, mock_otp_tab):
        """Test KnowledgeGraphTab structure and loading."""
        mock_services_tab.side_effect = lambda *args, **kwargs: Container()
        mock_otp_tab.side_effect = lambda *args, **kwargs: Container()
        app = AgentTUI(project_dir=self.project_dir)
        async with app.run_test() as pilot:
...
class TestTUIComponents(unittest.IsolatedAsyncioTestCase):
    ...
    def test_knowledge_tab_load(self, MockKnowledgeManager):
        ...
        # Mock query_one to return list view
        mock_list = MagicMock(spec=ListView)
        tab.query_one = MagicMock(side_effect=lambda selector, type=None: mock_list if selector == "#kg-node-list" else MagicMock())
