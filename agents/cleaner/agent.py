...
async def run_cleaner_agent(config: Config, agent_client=None):
    ...
    if config.agent_type == "cursor":
        ...
        run_session = run_cursor_session
        logger.info("Using Cursor Client for Cleanup")
    else:
        # Default to Gemini
        client = GeminiClient(config)
        run_session = run_gemini_session
        logger.info("Using Gemini Client for Cleanup")
    ...
    for i in range(max_cleaner_iterations):
        ...
        # Ensure we pass the correct client to the correct session runner
        # Although Python is duck-typed, it's safer to keep them aligned
        status, response, new_actions = await run_session(
            client,
            prompt,
            recent_history,
