...
try:
    from agents.cursor.agent import run_agent_session as run_cursor_session, CursorClient
except ImportError:
...
class SprintManager:
    ...
    async def run_planning_phase(self):
        ...
        if not search_path.exists():
            logger.warning("sprint_plan.json file not found. Attempting to parse from response text...")
            ...
            if match:
                try:
                    ...
                    search_path.write_text(json_content) # Write it to file for consistency
                    logger.info("Successfully recovered sprint plan from response text.")
                except Exception as e:
                    logger.error(f"Failed to extract plan from response: {e}")
                    return False
            else:
                logger.error("Sprint Plan not created and no JSON block found. Aborting.")
        ...
        try:
            ...
            self.plan = SprintPlan(sprint_goal=plan_data.get("sprint_goal", ""), tasks=tasks)
    ...
    async def run_worker(self, task: Task):
        ...
        if self.agent_client:
             self.agent_client.report_state(current_task=f"Spawning Worker: {task.title}")
        ...
        worker_client.report_state(is_running=True, current_task=f"Starting Task: {task.title}", iteration=0)
        ...
        try:
            while turns < max_turns:
                turns += 1
                worker_client.report_state(iteration=turns, current_task=f"Executing: {task.title}")
                ...
                if "SPRINT_TASK_COMPLETE" in response:
                    ...
                    worker_client.report_state(current_task="Completed", is_running=False)
                    worker_client.stop()
                    return
    ...
    async def execute_sprint(self):
        ...
        while len(self.completed_tasks) + len(self.failed_tasks) < len(self.plan.tasks):
            ...
            if not self.running_tasks and not runnable and len(self.completed_tasks) < len(self.plan.tasks):
                 logger.error("Deadlock detected? No running tasks and no runnable tasks.")
                 break
