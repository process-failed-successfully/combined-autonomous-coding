import { Play, Pause, Square, SkipForward, Activity, Power, Clock, Maximize2, Minimize2 } from 'lucide-react';
...
export function AgentCard({ agent, onControl, isFocused, onToggleFocus }: AgentCardProps) {
    ...
    // Dynamic height based on focus mode
    const cardHeight = isFocused ? "h-[80vh]" : "";

    return (
        <div className={clsx("bg-dark-700 border border-dark-600 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col group", cardHeight)}>
            ...
            <div className="p-5 flex-grow space-y-4 flex flex-col min-h-0">
                ...
                {/* Ensure terminal grows and scrolls */}
                <div className="flex-grow min-h-0 flex flex-col">
                    <Terminal logs={agent.last_log} />
                </div>
            </div>

            <div className="p-4 bg-dark-800 border-t border-dark-600 flex gap-2 flex-shrink-0">
                {agent.is_paused ? (
                    <button 
