...
class TestTUIDocs(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patches = [
            ...
            # patch('shared.tui.get_all_log_files', return_value=[]), # Removed as it's not used in TUI directly
            patch('shared.tui.get_git_info', return_value={"branch": "main", "status": "Clean"}),
            patch('shared.tui.get_workflow_stage', return_value="Dev"),
            # patch('shared.tui.get_git_log', return_value=[]), # Removed as it's not used in TUI directly
            patch('shared.tui.scan_project', return_value={}),
            patch('shared.tui.RecipeManager'),
            patch('shared.tui.WorktreeManager'),
            # patch('shared.tui.DependencyAnalyzer'), # Removed as it's not used in TUI directly
            # patch('shared.tui.DependencyUpdater'), # Removed as it's not used in TUI directly
