...
def run_zlib_lab(args):
    ...
    if getattr(args, "tui", False):
        from shared.tui import AgentTUI
        print("Launching Zlib Lab TUI...")
        app = AgentTUI(project_dir=getattr(args, 'project_dir', None), start_tab="tab-zlib")
        import asyncio
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
...
def run_magic_decode_lab(args):
    """Runs the Magic Decode Lab."""
    if getattr(args, "tui", False):
        from shared.tui import AgentTUI
        print("Launching Magic Decode Lab TUI...")
        app = AgentTUI(project_dir=getattr(args, 'project_dir', None), start_tab="tab-magic-decode")
        import asyncio
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
...
def run_fs_lab(args):
    ...
    if getattr(args, "action", None) == "tui" or getattr(args, "tui", False):
        from shared.tui import AgentTUI
        print("Launching FS Lab TUI...")
        app = AgentTUI(project_dir=getattr(args, 'project_dir', None), start_tab="tab-fs")
        import asyncio
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
...
def run_md2html_lab(args):
    ...
    if getattr(args, "action", None) == "tui" or getattr(args, "tui", False):
        from shared.tui import AgentTUI
        print("Launching Md2Html Lab TUI...")
        app = AgentTUI(project_dir=getattr(args, 'project_dir', None), start_tab="tab-md2html")
        import asyncio
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
...
def run_json2xml_lab(args):
    ...
    if getattr(args, "action", None) == "tui" or getattr(args, "tui", False):
        from shared.tui import AgentTUI
        print("Launching Json2Xml Lab TUI...")
...
def parse_args(argv=None):
    ...
    parser_magic_decode = subparsers.add_parser(
        "magic-decode-lab",
        aliases=["magic-decode", "mdecode"],
        help="Automatically decode an opaque string"
    )
    parser_magic_decode.add_argument("--text", help="Text to decode")
    parser_magic_decode.add_argument("--tui", action="store_true", help="Launch interactive TUI")
    ...
    # md2html-lab
    parser_md2html = subparsers.add_parser(
        "md2html-lab",
        aliases=["md2html"],
        help="Convert Markdown to HTML.",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser_md2html.add_argument("--file", help="Input Markdown file.")
    parser_md2html.add_argument("--text", help="Input Markdown text.")
    parser_md2html.add_argument("--output", "-o", help="Output file path.")
    parser_md2html.add_argument("--tui", action="store_true", help="Launch Markdown to HTML TUI.")
    parser_md2html.set_defaults(action="convert")
    ...
    jmespath_eval_parser.add_argument("input", help="Input JSON file path or '-' for stdin.")
    ...
    parser_xml2csv.add_argument("--text", "-t", help="Input XML text.")
    ...
    parser_yaml2toml = subparsers.add_parser(
        ...
        help="YAML to TOML and TOML to YAML Converter Lab"
    )
    yaml2toml_subparsers = parser_yaml2toml.add_subparsers(
        dest="action",
        help="Action to perform."
    )
    yaml2toml_subparsers.add_parser("tui", help="Launch YAML2TOML Lab TUI.")

    yaml2toml_parser_y2t = yaml2toml_subparsers.add_parser("yaml2toml", help="Convert YAML to TOML.")
    yaml2toml_parser_y2t.add_argument("input", help="YAML string or file path.")
    yaml2toml_parser_y2t.add_argument("--output", "-o", help="Output file path.")

    yaml2toml_parser_t2y = yaml2toml_subparsers.add_parser("toml2yaml", help="Convert TOML to YAML.")
    yaml2toml_parser_t2y.add_argument("input", help="TOML string or file path.")
    yaml2toml_parser_t2y.add_argument("--output", "-o", help="Output file path.")
    ...
    parser_csv2json = subparsers.add_parser(
        "csv2json-lab",
        aliases=["c2j"],
        help="CSV to JSON converter utilities."
    )
    ...
    parser_csv2yaml = subparsers.add_parser(
        ...
        help="CSV to YAML converter utilities."
    )
    ...
    parser_json2swift = subparsers.add_parser(
        ...
        help="JSON to Swift Lab: Convert JSON into Swift structs."
    )
    parser_json2swift.add_argument("--file", "-f", help="Input JSON file.")
    parser_json2swift.add_argument("--text", "-t", help="Input JSON text.")
    parser_json2swift.add_argument("--output", "-o", help="Output Swift file path.")
    parser_json2swift.add_argument("--name", "-n", default="RootStruct", help="Root struct name (default: 'RootStruct').")
    parser_json2swift.add_argument("--tui", action="store_true", help="Launch JSON to Swift Lab TUI.")
    ...
    parser_csv2html.add_argument("--tui", action="store_true", help="Launch the CSV to HTML TUI.")
      ...
      # --- New 'csv2sql-lab' command ---
    parser_csv2sql = subparsers.add_parser(
        "csv2sql-lab",
        aliases=["csv2sql", "c2s"],
        help="Convert CSV data to SQL INSERT statements."
    )
    parser_csv2sql.add_argument("--file", "-f", help="The input CSV file path.")
    parser_csv2sql.add_argument("--text", "-t", help="The input CSV text string.")
    parser_csv2sql.add_argument("--table", help="Target SQL table name.", default="mytable")
    parser_csv2sql.add_argument("--output", "-o", help="Optional output file to write SQL statements.")
    parser_csv2sql.add_argument("--tui", action="store_true", help="Launch the CSV to SQL Lab TUI.")

    # --- New 'json2sql-lab' command ---
    parser_json2sql = subparsers.add_parser(
        "json2sql-lab",
        aliases=["json2sql", "j2s"],
        help="Convert JSON data to SQL INSERT statements."
    )
    parser_json2sql.add_argument("--file", "-f", help="The input JSON file path.")
    parser_json2sql.add_argument("--text", "-t", help="The input JSON text string.")
    parser_json2sql.add_argument("--table", help="Target SQL table name.", default="mytable")
    parser_json2sql.add_argument("--output", "-o", help="Optional output file to write SQL statements.")
    parser_json2sql.add_argument("--tui", action="store_true", help="Launch the JSON to SQL Lab TUI.")
    # --- New 'csv-lab' command ---
    ...
    parser_json2xml.add_argument("--file", "-f", help="Input JSON file.")
    parser_json2xml.add_argument("--text", "-t", help="Input JSON string.")
    parser_json2xml.add_argument("--output", "-o", help="Output file to save the XML.")
    parser_json2xml.add_argument("--tui", action="store_true", help="Launch interactive JSON to XML Lab.")
...
async def main():
    ...
    if args.command in ["md2html-lab", "md2html"]:
        run_md2html_lab(args)
        return
    ...
    if args.command in ["xml2yaml-lab", "xml2yaml", "x2y"]:
        ...
        if getattr(args, "tui", False):
            run_tui(args, "tab-xml2yaml")
            return
        run_xml2yaml_lab_logic(args)
        return

    if args.command in ["yaml2xml-lab", "yaml2xml", "y2x"]:
        args.action = "yaml2xml"
        if getattr(args, "tui", False):
            run_tui(args, "tab-xml2yaml")
            return
        run_xml2yaml_lab_logic(args)
    ...
    if args.command in ["csv2sql-lab", "csv2sql", "c2s"]:
        if getattr(args, "tui", False) or getattr(args, "action", None) == "tui":
            from shared.tui import AgentTUI
            print("Launching Csv2Sql Lab TUI...")
            app = AgentTUI(project_dir=getattr(args, 'project_dir', Path(".")), start_tab="tab-csv2sql")
            import asyncio
            try:
                loop = asyncio.get_running_loop()
            except RuntimeError:
    ...
    if args.command in ["json2sql-lab", "json2sql", "j2s"]:
        from shared.json2sql_lab import run_json2sql_lab_logic
