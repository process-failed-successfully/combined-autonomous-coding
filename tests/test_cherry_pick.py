...
class TestCherryPickCommand(unittest.TestCase):
    ...
    @patch('sys.stdout')
    @patch('sys.stderr')
    def test_cherry_pick_invalid_ref(self, mock_stderr, mock_stdout):
        """Test cherry-picking with an invalid and potentially malicious ref."""
        args = MagicMock()
        args.project_dir = self.test_dir
        args.target = "--dummy-option"
        ...
        self.assertEqual(cm.exception.code, 1)
        stderr_output = "".join(call.args[0] for call in mock_stderr.write.call_args_list)
        self.assertIn("Error: Invalid or unsafe target provided", stderr_output)
