...
class PresentationTab(Container):
    ...
    async def generate_presentation(self) -> None:
        ...
        # We pass arguments to instantiate generator inside the thread
        success = await asyncio.to_thread(self._run_generation, self.project_dir, agent_type, output_path, theme)
    ...
    def _run_generation(self, project_dir: Path, agent_type: str, output_path: Path, theme: str) -> bool:
        """Helper to run the async generate method synchronously for to_thread."""
        import asyncio

        # Instantiate inside the thread to ensure it uses the correct loop if it creates one or checks one
        generator = PresentationGenerator(project_dir, agent_type=agent_type)
