...
def run_cherry_pick(args):
    ...
    # Use is_safe_git_ref to prevent argument injection
    ...
    if not is_valid_ref:
        ...
        commit_hash = find_commit_by_run_id(project_dir, target, git_path)
        if commit_hash:
...
def run_diff(args):
    ...
    try:
        ...
        is_git_ref = False
        if is_safe_git_ref(target):
            # We must validate it's a safe ref before passing to rev-parse or show
