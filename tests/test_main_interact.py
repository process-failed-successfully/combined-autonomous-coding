...
class TestInteractCommand(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('main.run_commit', autospec=True)
    async def test_interact_chooses_commit(self, mock_run_commit, mock_input):
        ...
        # Ensure mock_run_commit is treated as a coroutine function
        # unittest.mock.patch should handle this if main.run_commit is async,
        # but to be safe we can ensure it behaves like one or check if it works.
