...
from textual.widgets import Static, Button, TextArea, Input, Label, TabPane
from textual.binding import Binding
from shared.json2swift_lab import Json2SwiftManager

class Json2SwiftLabTab(TabPane):
    ...
    def __init__(self, **kwargs):
        # We initialize it with the required title argument for TabPane
        super().__init__("Json2Swift", id="tab-json2swift", **kwargs)
        self.manager = Json2SwiftManager()

    def compose(self) -> ComposeResult:
        with Vertical(id="json2swift-container", classes="p-1"):
            yield Static("JSON to Swift Structs Converter", classes="text-bold text-center p-1 bg-primary text-primary-content")
            ...
            with Horizontal(classes="h-full"):
                with Vertical(classes="w-1-2 p-1 border-r border-primary h-full"):
                    ...
                    yield TextArea(id="json2swift-input-ta", language="json", classes="h-full")

                with Vertical(classes="w-1-2 p-1 h-full"):
                    yield Label("Output Swift:", classes="text-bold mb-1")
                    # Note: We use language="swift" if supported, else "text" or "c"
                    yield TextArea(id="json2swift-output-ta", language="swift", classes="h-full", read_only=True)
