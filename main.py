...
# --- New 'argon2-lab' command ---
    ...
    favicon_lab_parser = subparsers.add_parser(
        "favicon-lab",
        aliases=["favicon"],
        help="Generate standard web favicons from an image."
    )
    favicon_lab_parser.add_argument("action", nargs="?", choices=["generate", "html", "tui"], help="Action to perform: 'generate', 'html', or 'tui'.")
    favicon_lab_parser.add_argument("--image", help="Path to the source image (must be at least 512x512). Required for 'generate'.")
    favicon_lab_parser.add_argument("--output", default=".", help="Output directory for generated favicons. Defaults to current directory.")
    favicon_lab_parser.add_argument("--tui", action="store_true", help="Launch the Favicon Lab TUI.")

    # --- New 'json2ini-lab' command ---
    parser_json2ini = subparsers.add_parser(
        "json2ini-lab",
        aliases=["json2ini", "j2i"],
        help="JSON to INI converter utilities."
    )
    parser_json2ini.add_argument("--file", "-f", help="Input JSON file.")
    parser_json2ini.add_argument("--text", "-t", help="Input JSON text.")
