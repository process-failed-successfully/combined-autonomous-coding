...
class DiskUsageTab(Container):
    ...
    def _update_tree(self, data: Dict[str, Any]) -> None:
        ...
        # Ensure size is an int before formatting, or handle default
        size_val = data.get("size", 0)
        root_size = format_size(int(size_val) if isinstance(size_val, (int, float)) else 0)
        ...
        if isinstance(children, list):
             self._add_children(tree.root, children)

    def _add_children(self, node: Any, children: List[Dict[str, Any]]) -> None:
        """Adds immediate children to the tree node."""
        for child in children:
            size_val = child.get("size", 0)
            size_str = format_size(int(size_val))
    ...
    def _update_table(self, files: List[Dict[str, Any]]) -> None:
        ...
        for f in files:
            ...
            size_fmt = str(f.get("formatted_size", ""))
            table.add_row(size_fmt, str(rel_path), key=str(path_obj))
    ...
    def on_tree_selected(self, event: Tree.NodeSelected) -> None:
        ...
        self.selected_path = node_data.get("path")
