...
class TestIterationMetrics(unittest.TestCase):
    def setUp(self):
        # Reset telemetry
        self.telemetry = init_telemetry("test_service", agent_type="test", project_name="test_project")
    ...
    def test_iteration_metrics_registered(self):
        ...
        for metric in expected_metrics:
            self.assertIn(metric, self.telemetry.metrics, f"Metric {metric} not registered")
...
if __name__ == '__main__':
    ...
    for arg in sys.argv[1:]:
        if not arg.startswith("--dashboard-url") and not arg.startswith("--no-dashboard"):
