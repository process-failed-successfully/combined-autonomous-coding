...
class TarLabApp(App):
    ...
    BINDINGS = [
        ...
        Binding("l", "focus_list", "Focus List"),
    ]
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            ...
            with Vertical(classes="panel", id="create-panel"):
                ...
                with Horizontal(classes="input-row"):
                    yield Label("Output Tar:", classes="input-label")
                    yield Input(placeholder="archive.tar.gz", id="create-output")
                with Horizontal(classes="input-row"):
                    yield Label("Compression:", classes="input-label")
                    yield Select((("gzip", "gz"), ("bzip2", "bz2"), ("xz", "xz"), ("none", "none")), id="create-compression", value="gz")
            ...
            with Vertical(classes="panel", id="extract-panel"):
                ...
                with Horizontal(classes="input-row"):
                    yield Label("Input Tar:", classes="input-label")
                    yield Input(placeholder="archive.tar.gz", id="extract-input")
                with Horizontal(classes="input-row"):
            ...
            with Vertical(classes="panel", id="list-panel"):
                yield Label("List Archive", classes="title")
                with Horizontal(classes="input-row"):
                    yield Label("Input Tar:", classes="input-label")
                    yield Input(placeholder="archive.tar.gz", id="list-input")
                yield Button("List", id="btn-list", variant="warning")
                yield Static("", id="list-status", classes="status-area")
        yield Footer()
    ...
    def action_focus_list(self):
        self.query_one("#list-input", Input).focus()
    ...
    def _handle_create(self) -> None:
        ...
        compression_val = self.query_one("#create-compression", Select).value
        ...
        output_path = Path(output_str) if output_str else Path("archive.tar.gz")
        ...
        try:
            final_path = self.manager.create(input_paths, output_path, compression)
    ...
    def _handle_list(self) -> None:
        input_str = self.query_one("#list-input", Input).value.strip()
        ...
        try:
            names = self.manager.list(input_path)
            content = "\n".join(names)
            status.update(f"[green]Archive Contents:[/green]\n{content}")
        except Exception as e:
            status.update(f"[red]Error:[/red]\n{e}")
