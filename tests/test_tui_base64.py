...
class TestBase64LabTab(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.patcher = patch("shared.tui_base64.DevTools")
        self.MockDevTools = self.patcher.start()

        self.tab = Base64LabTab()
    ...
    def test_process_encode(self):
        ...
        def query_side_effect(selector, type=None):
            if selector == "#b64-input":
                return input_area
            if selector == "#b64-output":
                return output_area
            return MagicMock()

        self.tab.query_one.side_effect = query_side_effect
        self.MockDevTools.base64_encode.return_value = "SGVsbG8="

        self.tab.process(encode=True)

        self.MockDevTools.base64_encode.assert_called_with("Hello")
    ...
    def test_process_decode(self):
        ...
        self.tab.query_one.side_effect = query_side_effect
        self.MockDevTools.base64_decode.return_value = "Hello"

        self.tab.process(encode=False)

        self.MockDevTools.base64_decode.assert_called_with("SGVsbG8=")
    ...
    def test_process_decode_error(self):
        ...
        self.tab.query_one.side_effect = query_side_effect
        self.MockDevTools.base64_decode.return_value = "Error: Invalid Base64"

        self.tab.process(encode=False)

        self.MockDevTools.base64_decode.assert_called_with("invalid_base64!!")
        self.assertEqual(output_area.text, "Error: Invalid Base64")
