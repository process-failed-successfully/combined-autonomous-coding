...
class TestGenerator:
    ...
    async def generate_test_code(
        ...
        """
        Generates test code for the target file using AI.
        Returns (success, code).
        """
        target_file = target_file.resolve()

        if not target_file.exists():
            return False, f"Error: Target file '{target_file}' does not exist."
        ...
        print(f"Requesting {framework} test generation for {target_file.name}...")
        try:
    ...
    async def generate_tests(
        ...
        success, content = await self.generate_test_code(
            target_file, framework, agent_type, model
        )
        ...
        if success:
             ...
             print(f"❌ Failed to generate tests: {content}")
             return False
