...
class TestCherryPickCommand(unittest.TestCase):
    ...
    def test_cherry_pick_invalid_target(self, mock_stderr, mock_stdout):
        """Test that cherry-pick rejects invalid git references."""
        args = MagicMock()
        args.project_dir = self.test_dir
        ...
        self.assertEqual(cm.exception.code, 1)
        stderr_output = "".join(call.args[0] for call in mock_stderr.write.call_args_list)
        # It should fail because it skips cat-file (unsafe) and fails run_id lookup
        self.assertIn("Error: Could not find a git commit for target '-invalid-flag'.", stderr_output)
