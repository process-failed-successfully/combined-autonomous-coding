...
import io
from main import get_parser, run_rewind
...
class TestRewindCommand(unittest.TestCase):
    ...
    def test_rewind_to_specific_commit(self):
        with self.assertRaises(SystemExit) as cm:
            parser = get_parser()
            args = parser.parse_args(["rewind", self.second_commit_hash, "--project-dir", str(self.project_dir), "--yes"])
            run_rewind(args)
    ...
    def test_rewind_relative_commit(self):
        with self.assertRaises(SystemExit) as cm:
            parser = get_parser()
            args = parser.parse_args(["rewind", "HEAD~1", "--project-dir", str(self.project_dir), "--yes"])
            run_rewind(args)
    ...
    def test_rewind_aborts_with_uncommitted_changes(self):
        ...
        with self.assertRaises(SystemExit) as cm, \
            ...
            parser = get_parser()
            args = parser.parse_args(["rewind", self.initial_commit_hash, "--project-dir", str(self.project_dir), "--yes"])
            run_rewind(args)
    ...
    def test_rewind_interactive_mode(self, mock_input):
        with self.assertRaises(SystemExit) as cm:
            parser = get_parser()
            args = parser.parse_args(["rewind", "--project-dir", str(self.project_dir)])
            run_rewind(args)
    ...
    def test_rewind_by_run_id(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            parser = get_parser()
            args = parser.parse_args(["rewind", run_id, "--project-dir", str(self.project_dir), "--yes"])
            run_rewind(args)
