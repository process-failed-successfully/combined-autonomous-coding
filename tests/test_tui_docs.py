...
class TestTUIDocs(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patches = [
            ...
            patch('shared.map.scan_project', return_value={}),
            patch('shared.recipes.RecipeManager'),
            patch('shared.worktree.WorktreeManager'),
            patch('shared.dependencies.DependencyAnalyzer'),
            patch('shared.dependencies.DependencyUpdater'),
            patch('shared.task_manager.TaskManager'),
            patch('shared.knowledge.KnowledgeManager'),
            patch('shared.database.init_db'), # prevent db init
            patch('shared.api_lab.ApiLabManager'),
            patch('shared.playground.PlaygroundManager'),
            patch('shared.secrets.SecretsManager'),
            patch('shared.troubleshoot.TroubleshootManager'),
            patch('shared.recipe_learner.RecipeLearner'),
            patch('shared.debt.DebtCollector'),
