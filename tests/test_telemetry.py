...
class TestTelemetry(unittest.TestCase):
    ...
    def test_record_gauge(self, mock_push):
        ...
        with patch("shared.telemetry.ENABLE_METRICS", True):
            ...
            # record_gauge automatically adds default labels if configured in _init_metrics
            # But since "test_metric" is custom, we must not use labels or ensure register matches.
            # However, the code under test (telemetry.py) tries to autofill labels based on what's registered.
            # Telemetry.register_gauge registers with empty labelnames by default if not provided.
