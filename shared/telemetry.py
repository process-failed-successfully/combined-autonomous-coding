...
class Telemetry:
    ...
    def __init__(self, service_name: str, job_name: str = "agent_job",
                 agent_type: str = "unknown", project_name: str = "unknown"):
    ...
    def _init_metrics(self):
        # 1. Agent Health
        self.register_gauge("agent_heartbeat_timestamp", "Unix timestamp of last heartbeat", ["agent_id", "project"])
        self.register_gauge("agent_online", "Binary (1=online, 0=offline)", ["agent_id", "project"])
        self.register_gauge("agent_uptime_seconds", "Time since agent started", ["agent_id", "project"])
        self.register_counter("agent_restart_total", "Number of restarts", ["agent_id", "project", "reason"])

        # 2. Progress
        self.register_gauge("feature_completion_pct", "Percentage of features passing", ["agent_id", "project"])
        self.register_gauge("features_passing", "Number of passing features", ["agent_id", "project"])
        ...
        self.register_counter("llm_tokens_total", "Combined token counter", ["agent_id", "model", "type"])
        self.register_counter("llm_errors_total", "LLM API errors", ["agent_id", "model", "error_type"])

        # 4. Tool Execution
        self.register_counter("tool_execution_total", "Tool invocations", ["agent_id", "tool_type"])
        self.register_histogram("tool_execution_duration_seconds", "Tool execution time", ["agent_id", "tool_type"], buckets=(0.1, 0.5, 1, 5, 10, 30, 60))
        self.register_counter("tool_errors_total", "Tool failures", ["agent_id", "tool_type", "error_type"])
        self.register_counter("files_written_total", "Files created/modified", ["agent_id", "project"])
        self.register_counter("files_read_total", "Files read", ["agent_id", "project"])
        self.register_counter("bash_commands_total", "Bash executions", ["agent_id", "project", "status"])

        # 5. Resource Usage
        self.register_gauge("container_memory_usage_bytes", "Memory consumption", ["agent_id", "project"])
        self.register_gauge("container_cpu_usage_pct", "CPU usage percentage", ["agent_id"])
        self.register_gauge("process_count", "Child processes spawned", ["agent_id"])

        # 6. Errors
        self.register_counter("agent_errors_total", "All agent errors", ["agent_id", "error_type"])
        self.register_counter("agent_crashes_total", "Agent process crashes", ["agent_id"])
    ...
    def register_counter(self, name: str, documentation: str, labelnames: list = []):
        if name not in self.metrics:
            self.metrics[name] = Counter(name, documentation, labelnames=labelnames, registry=self.registry)

    def register_histogram(self, name: str, documentation: str, labelnames: list = [], buckets: tuple = Histogram.DEFAULT_BUCKETS):
        if name not in self.metrics:
            self.metrics[name] = Histogram(name, documentation, labelnames=labelnames, registry=self.registry, buckets=buckets)
    ...
    def _get_labels(self, labels: Dict[str, str]) -> Dict[str, str]:
        ...
        if "agent_id" not in final_labels and "agent_id" in self.metrics[list(final_labels.keys())[0] if labels else ""]._labelnames:
    ...
    def record_gauge(self, name: str, value: float, labels: Dict[str, str] = None):
        ...
        if name in self.metrics:
            ...
            if not required_labels:
                # If no labels are required, we should call set directly on the metric
                # However, our design seems to imply we always want labels.
                # If a metric was registered without labels, Prometheus client expects direct set()
    ...
    def _system_monitoring_loop(self):
        while self.monitoring_active:
            try:
                ...
                self.record_gauge("process_count", len(process.children(recursive=True)) + 1) # Self + children
...
def init_telemetry(service_name: str, agent_type: str = "unknown", project_name: str = "unknown"):
    global _telemetry
    _telemetry = Telemetry(service_name, agent_type=agent_type, project_name=project_name)
    return _telemetry
